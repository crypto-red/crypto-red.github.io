...
import {loadJSON, loadValueHtmlSelector, postDATA} from "../utils/load-json";
...
function _get_doge_account_by_seed(seed) {
...
function get_doge_fee_per_byte(callback_function) {
    ...
    loadJSON("https://api.blockchair.com/dogecoin/stats", extract_value)
}


function get_doge_address_by_seed(seed) {

    let { child } = _get_doge_account_by_seed(seed);
    return bitcoin.payments.p2pkh({ pubkey: child.publicKey, network: NETWORK }).address;
}

function get_doge_public_key_by_seed(seed) {

    let { child } = _get_doge_account_by_seed(seed);
    return child.publicKey;
}

function _format_doge_amount(amount) {

    return amount * 1;
}

function _format_doge_attachment(attachment) {

    const attachment_bytes = base58.decode(attachment);
    return converters.byteArrayToString(attachment_bytes);
}

function send_doge_transaction(seed, address, amount, memo, callback_function, return_fee_instead_of_send = false) {

    const my_address = get_doge_address_by_seed(seed);
    const { key_pair } = _get_doge_account_by_seed(seed);
                            ...
                            if(return_fee_instead_of_send) {
                                ...
                                if(amount_satoshis + txfee <= balance) {
                                    ...
                                    try {
                                        ...
                                        postDATA("https://chain.so/api/v2/send_tx/DOGETEST", "tx_hex=" + tx_hex, send_transaction_callback);
                                    ...
                                    callback_function("Not enough dogecoins", null);
                                }
                            }
    ...
    loadJSON('https://chain.so/api/v2/get_tx_unspent/DOGETEST/' + my_address, add_unspent_output);
}


function get_doge_account_balance_by_seed(parameters, callback_function) {

    const { seed } = parameters;
    const my_address = get_doge_address_by_seed(seed);

    function get_doge_balance_callback(error, response) {

        if(!error) {

            if(response.status !== "fail") {

                callback_function(null, _format_doge_amount(response.data.confirmed_balance));
            }else {
    ...
    loadJSON("https://chain.so/api/v2/get_address_balance/DOGETEST/"+my_address+"/1", get_doge_balance_callback)

}

function get_doge_account_transactions_by_seed(parameters, callback_function) {
    ...
    function format_get_doge_transaction_response(error, response, direction) {
    ...
    loadJSON("https://chain.so/api/v2/get_tx_received/DOGETEST/" + my_address + "/" + after_transaction_id, (error, response) => {format_get_doge_transaction_response(error, response, "received")});
    loadJSON("https://chain.so/api/v2/get_tx_spent/DOGETEST/" + my_address + "/" + after_transaction_id, (error, response) => {format_get_doge_transaction_response(error, response, "sent")});


}

function get_doge_transaction_by_id(parameters, callback_function) {

    const { id, seed } = parameters;
    const address = get_doge_address_by_seed(seed);

    function format_get_each_doge_transaction_response(error, response){
    ...
    loadJSON("https://chain.so/api/v2/get_tx/DOGETEST/"+id, format_get_each_doge_transaction_response);

}

function get_doge_send_transaction_info() {
...
module.exports = {
    get_doge_fee_per_byte: get_doge_fee_per_byte,
    get_doge_address_by_seed: get_doge_address_by_seed,
    get_doge_public_key_by_seed: get_doge_public_key_by_seed,
    send_doge_transaction: send_doge_transaction,
    get_doge_account_transactions_by_seed: get_doge_account_transactions_by_seed,
    get_doge_transaction_by_id: get_doge_transaction_by_id,
    get_doge_account_balance_by_seed: get_doge_account_balance_by_seed,
    get_doge_send_transaction_info: get_doge_send_transaction_info
};