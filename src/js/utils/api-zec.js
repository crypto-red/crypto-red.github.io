...
function _get_zec_account_by_seed(seed) {
...
function get_zec_fee_per_byte(callback_function) {
    ...
    loadJSON("https://api.blockchair.com/zcash/stats", extract_value)
}

function get_zec_address_by_seed(seed) {

    let { child } = _get_zec_account_by_seed(seed);
    return bitcoin.payments.p2pkh({ pubkey: child.publicKey, network: NETWORK }).address;
}

function get_zec_public_key_by_seed(seed) {

    let { child } = _get_zec_account_by_seed(seed);
    return child.publicKey;
}

function _format_zec_amount(amount) {

    return amount * 1;
}

function _format_zec_attachment(attachment) {

    const attachment_bytes = base58.decode(attachment);
    return converters.byteArrayToString(attachment_bytes);
}

function send_zec_transaction(seed, address, amount, memo, callback_function, return_fee_instead_of_send = false) {

    const my_address = get_zec_address_by_seed(seed);
    const { key_pair } = _get_zec_account_by_seed(seed);
                            ...
                            if(return_fee_instead_of_send) {
                                ...
                                if(amount_satoshis + txfee <= balance) {
                                    ...
                                    try {
                                        ...
                                        postDATA("https://chain.so/api/v2/send_tx/ZECTEST", "tx_hex=" + tx_hex, send_transaction_callback);
                                    ...
                                    callback_function("Not enough zcash", null);
                                }
                            }
    ...
    loadJSON('https://chain.so/api/v2/get_tx_unspent/ZECTEST/' + my_address, add_unspent_output);
}

function get_zec_account_balance_by_seed(parameters, callback_function) {

    const { seed } = parameters;
    const my_address = get_zec_address_by_seed(seed);

    function get_zec_balance_callback(error, response) {

        if(!error) {

            if(response.status !== "fail") {

                callback_function(null, _format_zec_amount(response.data.confirmed_balance));
            }else {
    ...
    loadJSON("https://chain.so/api/v2/get_address_balance/ZECTEST/"+my_address+"/1", get_zec_balance_callback)

}

function get_zec_account_transactions_by_seed(parameters, callback_function) {
    ...
    function format_get_zec_transaction_response(error, response, direction) {
    ...
    loadJSON("https://chain.so/api/v2/get_tx_received/ZECTEST/" + my_address + "/" + after_transaction_id, (error, response) => {format_get_zec_transaction_response(error, response, "received")});
    loadJSON("https://chain.so/api/v2/get_tx_spent/ZECTEST/" + my_address + "/" + after_transaction_id, (error, response) => {format_get_zec_transaction_response(error, response, "sent")});


}

function get_zec_transaction_by_id(parameters, callback_function) {

    const { id, seed } = parameters;
    const address = get_zec_address_by_seed(seed);

    function format_get_each_zec_transaction_response(error, response){
    ...
    loadJSON("https://chain.so/api/v2/get_tx/ZECTEST/"+id, format_get_each_zec_transaction_response);

}

function get_zec_send_transaction_info() {
...
module.exports = {
    get_zec_fee_per_byte: get_zec_fee_per_byte,
    get_zec_address_by_seed: get_zec_address_by_seed,
    get_zec_public_key_by_seed: get_zec_public_key_by_seed,
    send_zec_transaction: send_zec_transaction,
    get_zec_account_transactions_by_seed: get_zec_account_transactions_by_seed,
    get_zec_transaction_by_id: get_zec_transaction_by_id,
    get_zec_account_balance_by_seed: get_zec_account_balance_by_seed,
    get_zec_send_transaction_info: get_zec_send_transaction_info
};
