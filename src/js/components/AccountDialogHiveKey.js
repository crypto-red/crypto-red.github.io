...
class AccountDialogHiveKey extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            key_function: props.key_function,
            open: props.open,
            _accounts: [],
            _lookup_loading: false,
            _trying_hive_function: false,
            _account_input: null,
            _account_name_input: "",
    ...
    _handle_key_down_input_two = (event) => {
        ...
        if(event.keyCode == 13){
            ...
            this._try_hive_function();
        }
    };

    _try_hive_function = () => {

        const { _account_name_input, _account_password_input, key_function } = this.state
        actions.trigger_sfx("ui_loading");

        this.setState({_trying_hive_function: true}, () => {

            key_function(_account_name_input, _account_password_input, (err, res) => {

                if(err) {

                    console.log(err);
                    this.setState({_trying_hive_function: false, _is_account_password_error: true}, () => {
                    ...
                    this.setState({_trying_hive_function: false, _is_account_password_error: false, _account_name_input: "", _account_password_input: ""}, () => {
    ...
    render() {
        ...
        const { classes, key_type, open, _trying_hive_function } = this.state;
        ...
        const password_label = key_type === "POSTING" ? "Posting private key":
            key_type === "ACTIVE" ? "Active private key":
                "Unknown key";

        return (
            <div>
                <Backdrop className={classes.backdrop} open={_trying_hive_function}>
                ...
                >
                    ...
                    <DialogActions>
                        <Button color="primary" onClick={(event) => {this._try_hive_function()}}>
                            {t( "words.ok")}
                        </Button>
...
export default withStyles(styles)(AccountDialogHiveKey);
